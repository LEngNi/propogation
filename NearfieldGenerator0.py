...
plt.show()
plt.contourf(X,Y,orgi2, 50, cmap='rainbow')
plt.show()
"""
...
# Vx = Vx.reshape(Xrangeext*Yrangeext,1)
# Vy = Vy.reshape(Xrangeext*Yrangeext,1)
...
'''
Amp=np.abs(Eresult)
Pha=np.angle(Eresult)
plt.contourf(X,Y,Amp, 50, cmap='rainbow')
plt.show()
plt.contourf(X,Y,Pha, 50, cmap='rainbow')
plt.show()
